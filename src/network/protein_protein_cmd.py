...
def ComputePosNegPValue(node_id, pos_neighbor_count, neg_neighbor_count, pos_k_s_map, neg_k_s_map, N, s_0):
    ...
    # Compute the chi squared statistic (fisher's method)
...
def OpenGenesCsvFile(path, node_to_id, col_idxs):
...
def ChooseRandomSeedNodes(node_to_id, num_seed_genes = 100):
    random_nodes = set()
    random_ids = set()
    while len(random_ids) < num_seed_genes:
        random_node = random.choice(node_to_id.keys())
        random_nodes.add(random_node)
        random_ids.add(node_to_id[random_node])
    name_file = 'random' + str(num_seed_genes)
    print(num_seed_genes, 'random nodes generated')
    return random_ids, random_nodes, name_file

            

if __name__ == '__main__':
    #assert len(sys.argv) == 5, "must call command as python protein_protein_cmd.py ../DATA/new_edges_t0.5_binary ../DATA/seed_nodes.csv 1 2" (0 ) 

    thres_file = sys.argv[1]
    seed_nodes_file = sys.argv[2]

    col_num = int(sys.argv[3])
    num_col_to_take = int(sys.argv[4])

    # optional parameter to indicate number of random seed genes to pick 9otherwise use the specified column
    if len(sys.argv) == 6:
        num_random = int(sys.argv[5])
    else:
        num_random = 0

    col_idxs  = [col_num + x for x in range(num_col_to_take)]

    Graph, node_to_id, edge_to_sign = OpenThresholdedFile(thres_file)

    if num_random != 0:
        disease_genes, disease_nodes, name_file = ChooseRandomSeedNodes(node_to_id, num_random)
    else:
        disease_genes, name_file, num_seed_genes = OpenGenesCsvFile(seed_nodes_file, node_to_id, col_idxs)
    ...
    out_name = "".join(["out_", str(col_num), "_", name_file, ".txt"])
    WriteNewDiseasesToFile(new_disease_genes, node_to_id, out_name)
