...
if __name__ == '__main__':
    ...
    assert len(sys.argv) == 3, "must call command as python validation_start.py ../DATA/new_edges_t0.5_binary ../DATA/seed_nodes.csv"
    ...
    for i in [0]: #range(1,80):
        try:
            disease_genes, name_file, num_seed_genes = get_original_genes(seed_path, node_to_id, i)

            out_name_diseases_avg = "".join([os.path.join(result_path,name_file), "_diseases_start_avg", ".txt"])
            out_name_diseases_all = "".join([os.path.join(result_path,name_file), "_diseases_start_all", ".txt"])
            # out_name_go = "".join([os.path.join(result_path,name_file), "_start_go", ".txt"])

            if not os.path.isfile(out_name_diseases_all):
                print(out_name_diseases_all)

                gene_list = sorted(list(disease_genes))
    ...
    # # result_path = '../Results/out_1_cancer_seed_omim_v2.txt'
    # result_path = sys.argv[1]
    # print(result_path)
    # files_to_run = sorted(os.listdir(result_path))
    ...
    #         if not os.path.isfile(out_name_diseases_all):
    #             print(file)
    #             gene_list = read_results_file(os.path.join(result_path,file))
